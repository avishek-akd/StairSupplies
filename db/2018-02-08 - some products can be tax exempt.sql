ALTER TABLE TblProductType
	ADD COLUMN `isTaxExempt` TINYINT(4) NOT NULL DEFAULT 0 COMMENT '=1 product is exempt from sales tax' AFTER `canEditListPrice`
;


DROP PROCEDURE spPricePerAccountingType
;
...
BEGIN
	...
	--  Calculate Wood Finish and Cable Rail Finish separately because it makes the final query easier to understand
	SELECT Coalesce(Sum( (TblProducts.FinishPrice * FinishPriceMultiplier) * (1 - TblOrdersBOM_Items.DiscountPercent) * TblOrdersBOM_Items.quantityOrdered ), 0) INTO woodFinishPrice
	...
	SELECT Coalesce(Sum( (TblProducts.FinishPrice * FinishPriceMultiplier) * (1 - TblOrdersBOM_Items.DiscountPercent) * TblOrdersBOM_Items.quantityOrdered ), 0) INTO cableRailFinish
	FROM TblOrdersBOM_ShipmentsItems
		INNER JOIN TblOrdersBOM_Items ON TblOrdersBOM_Items.OrderItemsID = TblOrdersBOM_ShipmentsItems.OrderItemsID
		INNER JOIN TblProducts        ON TblProducts.productID = TblOrdersBOM_Items.ProductID
		INNER JOIN TblProductType     ON TblProductType.ProductType_id = TblProducts.ProductType_id
