...
BEGIN
	...
	SELECT TblProductTypeAccountingType.id, title,
		CASE
			WHEN id = 2  THEN Coalesce(totalLineItems - woodFinishPrice, 0)
			WHEN id = 5  THEN Coalesce(totalLineItems - cableRailFinish, 0)
			WHEN id = 7  THEN Coalesce(woodFinishPrice, 0)
			WHEN id = 8  THEN Coalesce(cableRailFinish, 0)
			WHEN id = 9  THEN Coalesce(totalLineItems * (SELECT SalesTaxRate FROM TblOrdersBOM WHERE TblOrdersBOM.OrderID = orderID), 0)
			WHEN id = 10 THEN Coalesce((SELECT FreightCharge FROM TblOrdersBOM WHERE TblOrdersBOM.OrderID = orderID), 0)
			ELSE Coalesce(totalLineItems, 0)
		END AS total
	FROM TblProductTypeAccountingType
		LEFT JOIN (
						SELECT AccountingTypeID,
							Sum( vOrderItemPrice.unitPriceDiscounted * OBOMI.quantityOrdered ) AS totalLineItems
						FROM TblOrdersBOM_Items AS OBOMI
							INNER JOIN TblProducts     ON OBOMI.ProductID = TblProducts.productID
							INNER JOIN TblProductType  ON TblProductType.ProductType_id = TblProducts.ProductType_id
